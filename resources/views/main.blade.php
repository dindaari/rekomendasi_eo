@extends('layout.page')

@section('content')
  <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0"><strong>Welcome!</strong></h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Dashboard</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
    <!-- /.content-header -->

    <!-- Main content -->
    <section class="content">
      <div class="container-fluid">
        <!-- Small boxes (Stat box) -->
        <div class="row">
          <div class="col-lg-3 col-6">
            <!-- small box -->
            <div class="small-box bg-info">
              <div class="inner">
                <h4>Perusahaan</h4>

                <p>Data Perusahaan</p>
              </div>
              <div class="icon">
                <i class="ion ion-bag"></i>
              </div>
              <a href="{{ url('perusahaan') }}" class="small-box-footer">Informasi <i class="fas fa-arrow-circle-right"></i></a>
            </div>
          </div>
          <!-- ./col -->
          <div class="col-lg-3 col-6">
            <!-- small box -->
            <div class="small-box bg-success">
              <div class="inner">
                <h4>Kriteria</h4>

                <p>Kriteria</p>
              </div>
              <div class="icon">
                <i class="ion ion-stats-bars"></i>
              </div>
              <a href="{{ url('kriteria') }}" class="small-box-footer">Informasi <i class="fas fa-arrow-circle-right"></i></a>
            </div>
          </div>
          <!-- ./col -->
          <div class="col-lg-3 col-6">
            <!-- small box -->
            <div class="small-box bg-warning">
              <div class="inner">
                <h4>Alternatif</h4>

                <p>Alternatif</p>
              </div>
              <div class="icon">
                <i class="ion ion-person-add"></i>
              </div>
              <a href="{{ url('alternatif') }}" class="small-box-footer">Informasi <i class="fas fa-arrow-circle-right"></i></a>
            </div>
          </div>
          <!-- ./col -->
          <div class="col-lg-3 col-6">
            <!-- small box -->
            <div class="small-box bg-danger">
              <div class="inner">
                <h4>Hasil</h4>

                <p>Perhitungan</p>
              </div>
              <div class="icon">
                <i class="ion ion-pie-graph"></i>
              </div>
              <a href="{{ url('hitung') }}" class="small-box-footer">Informasi <i class="fas fa-arrow-circle-right"></i></a>
            </div>
          </div>
      
          <!-- ./col -->
        </div>
      </div><!-- /.container-fluid -->
      <div>
      <h4>Nilai Rating</h4>
        <table class="table">
          <thead>
            <tr>
              <th scope="col">Nilai</th>
              <th scope="col">Level Rating</th>
              
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">1</th>
              <td>Sangat Tidak Baik</th>              
            </tr>
            <tr>
              <th scope="row">2</th>
              <td>Rendah</td> 
            </tr>
            <tr>
              <th scope="row">3</th>
              <td>Cukup</td>
            </tr>
            <tr>
              <th scope="row">4</th>
              <td>Baik</td>
            </tr>
            <tr>
              <th scope="row">5</th>
              <td>Sangat Baik</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div>
        <h4>Tingkat Lokasi</h4>
        <table class="table">
          <thead>
            <tr>
              <th scope="col">Nilai</th>
              <th scope="col">Level Lokasi</th>
              
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">1</th>
              <td>Sangat Dekat</th>              
            </tr>
            <tr>
              <th scope="row">2</th>
              <td>Dekat</td> 
            </tr>
            <tr>
              <th scope="row">3</th>
              <td>Cukup Dekat</td>
            </tr>
            <tr>
              <th scope="row">4</th>
              <td>Tidak Dekat</td>
            </tr>
            <tr>
              <th scope="row">5</th>
              <td>Sangat Tidak Dekat</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div>
        <h4>Tingkat Kualitas</h4>
        <table class="table">
          <thead>
            <tr>
              <th scope="col">Kualitas</th>
              <th scope="col">Level Kualitas</th>
              
            </tr>
          </thead>
          <tbody>
            <tr>
              <th scope="row">1</th>
              <td>Sangat Tidak Baik</th>              
            </tr>
            <tr>
              <th scope="row">2</th>
              <td>Tidak Baik</td> 
            </tr>
            <tr>
              <th scope="row">3</th>
              <td>Cukup Baik</td>
            </tr>
            <tr>
              <th scope="row">4</th>
              <td>Baik</td>
            </tr>
            <tr>
              <th scope="row">5</th>
              <td>Sangat Baik</td>
            </tr>                        
          </tbody>
        </table>
      </div>
    </section>
    <!-- /.content -->
  </div>
  <!-- /.content-wrapper -->
  @endsection
  